...
export async function getServerSideProps(context) {
    ...
    if (!response.results || response.results.length === 0) {
        return { props: { serie: null, trailerKey: null, watchProviders: [] } };
    }

    const serie = response.results[0];

    const [trailerResponse, watchProvidersResponse, creditsResponse] = await Promise.all([
        get(`https://api.themoviedb.org/3/tv/${serie.id}/videos?language=pt-BR`),
        get(`https://api.themoviedb.org/3/tv/${serie.id}/watch/providers?language=pt-BR`),
        get(`https://api.themoviedb.org/3/tv/${serie.id}/credits?language=pt-BR`),
    ]);
