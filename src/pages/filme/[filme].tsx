...
import type { TMDBMovie, TMDBCastMember, TMDBWatchProvider, TMDBVideo } from '@/src/types/tmdb';
...
const MovieDetail = ({ movie, trailerKey, watchProviders, inTheaters, cast }: MovieDetailProps) => {
...
export async function getServerSideProps(context: GetServerSidePropsContext): Promise<GetServerSidePropsResult<MovieDetailProps>> {
  ...
  const isPlaying = nowPlayingResponse?.results?.some((m: TMDBMovie) => m.id === movie.id);
